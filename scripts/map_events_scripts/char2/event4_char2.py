...
def Start(cont):
    own = cont.owner
    event_children = MapEventsChildrens(own, 2, prices[3]) #There have modifications in each self script
...
def Update(cont):
    ...
    own.OpenCloseBoard(col_cursor, current_scene,
                       logic.getCurrentScene().objects["event_board.003"]) # setting object board
    ...
    if ((confirm) and (col_cursor.positive == True) and (own["active"] == True)):
        own.OnlyAddScene("talk_saci") #Name of talk scene 
