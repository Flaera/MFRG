...
func _ready():
	...
	car_loaded_instancied = car_loaded.instance()
...
func _process(_delta):
	get_node("ViewportContainer/Viewport/Spatial/Spatial").rotate_y(rotate_y_angle)
	rotate_y_angle = 0.4*_delta#lerp(rotate_y,360,10*_delta)
	#if (rotate_y >= 360.0):
	#	rotate_y=0.0
