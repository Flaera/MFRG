...
func _physics_process(delta):
	...
	if car_phys.getMove()==true:
		...
		if (calc[2]>0.0 and nitro==true):
			get_node("caixa_invoker_nitro/CPUParticles").lifetime = 5
			get_node("caixa_invoker_nitro001/CPUParticles").lifetime = 5
			get_node("caixa_invoker_nitro/CPUParticles").emitting = true
			get_node("caixa_invoker_nitro001/CPUParticles").emitting = true
		else:
			get_node("caixa_invoker_nitro/CPUParticles").lifetime = 0.01
			get_node("caixa_invoker_nitro001/CPUParticles").lifetime = 0.01
			get_node("caixa_invoker_nitro/CPUParticles").emitting = false
			get_node("caixa_invoker_nitro001/CPUParticles").emitting = false
		#Dust particles:
		if (rpm_medium>10):
