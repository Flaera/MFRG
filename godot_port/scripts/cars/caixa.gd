...
func _physics_process(delta):
	...
	if car_phys.getMove()==true:
		...
		if (calc[2]>0.0 and nitro==true):
			...
			get_node("caixa_invoker_nitro").visible = true
			get_node("caixa_invoker_nitro001").visible = true
		else:
			get_node("caixa_invoker_nitro/CPUParticles").lifetime = 0.01
			get_node("caixa_invoker_nitro001/CPUParticles").lifetime = 0.01
			get_node("caixa_invoker_nitro").visible = false
			get_node("caixa_invoker_nitro001").visible = false
		#Dust particles:
		if (rpm_medium>10):
