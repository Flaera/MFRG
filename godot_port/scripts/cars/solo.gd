...
func _ready():
	var data: Array = load("res://data_files/cars_specs.gd").new().specs["solo"]
	print(data)
...
func _physics_process(delta):
	...
	if car_phys.getMove()==true:
		...
		if (calc[2]>0.0 and nitro==true):
			get_node("solo_invoker_nitro/CPUParticles").emitting = true
			get_node("solo_invoker_nitro2/CPUParticles").emitting = true
		else:
			get_node("solo_invoker_nitro/CPUParticles").emitting = false
			get_node("solo_invoker_nitro2/CPUParticles").emitting = false
		#Dust particles:
		if (rpm_medium>10):
