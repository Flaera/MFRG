...
def Start(cont):
    own = cont.owner
    event_children = MapEventsChildrens(own, 1, prices[2]) #There have modifications in each self script
...
def Update(cont):
    ...
    own.OpenCloseBoard(col_cursor, current_scene,
                       logic.getCurrentScene().objects["event_board.002"]) # setting object board
    ...
    if ((confirm) and (col_cursor.positive == True) and (own["active"] == True)):
        own.OnlyAddScene("talk_moacir") #Name of talk scene 
        own.OnlyRemoveScenes(cont, [cont.actuators["re_map"],
         cont.actuators["re_map_smh"], cont.actuators["re_gold"], cont.actuators["re_mp"]])
